...
class Customer extends Component {
    ...
    const customerMonthsData = Array.from([
      ...new Set(
        customerData.map((transaction) =>
          transaction.transactionDate.toLocaleString("default", { month: "long" })
        )
      ),
    ]).map((mon) => {
      const custData = customerData.filter((transaction) => {
        return (
          transaction.transactionDate.toLocaleString("default", {
            month: "long",
          }) === mon
        );
      });
      return {
        month: mon,
        amount: custData.reduce((sum, current) => {
  ...
  tableData = () => {
    return this.state.data.map((dt, key) => {
      const { month, amount, 'Total Rewards': totalRewards } = dt;
      return (
        <tr key={month}>
          <td>{month}</td>
          <td>$ {amount}</td>
